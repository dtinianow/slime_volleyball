...
Slime.prototype.updatePosition = function(moveLeft, moveRight, jump) {
  for (var key in this.keysDown) {
    ...
    this.ifShouldMoveRight(value, moveRight);
    this.ifShouldMoveLeft(value, moveLeft);
    this.ifShouldJump(value, jump);
    this.ifShouldBeJumping(value, jump);
    this.ifStayStill();
  }
  return this;
};

Slime.prototype.ifShouldMoveRight = function(value, moveRight) {
  if (value == moveRight && this.isNotTouchingRightWall() && this.isNotTouchingLeftSideOfNet()) {
    this.moveToRight();
  }
}

Slime.prototype.ifShouldMoveLeft = function(value, moveLeft){
  if (value == moveLeft && this.isNotTouchingLeftWall() && this.isNotTouchingRightSideOfNet()) {
    this.moveToLeft();
  }
}

Slime.prototype.ifShouldJump = function(value, jump) {
  if (value == jump && this.isOnTheGround() && !this.jumping) {
    this.finishJump();
  }
}

Slime.prototype.ifShouldBeJumping = function(value, jump) {
...
Slime.prototype.ifStayStill = function() {
  this.move(0, 0);
}
